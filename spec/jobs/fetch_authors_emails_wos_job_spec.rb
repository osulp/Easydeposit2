...
RSpec.describe FetchAuthorsEmailsWosJob do
  ...
  it 'processes a successful job' do
    ...
    expect(user.events.first.name).to eq 'Fetch Authors Emails from Web of Science'
    expect(user.events.first.message).to eq 'Found 2 author emails in Web of Science full records.'
  ...
  it 'processes an job with no authors found' do
    ...
    expect(user.events.first.status).to eq 'completed'
    expect(user.events.first.name).to eq 'Fetch Authors Emails from Web of Science'
    expect(user.events.first.message).to eq 'Found 0 author emails in Web of Science full records.'
    expect(publication[:pub_at]).to be_falsey
  end
  it 'processes an error' do
    ...
    expect(user.events.first.name).to eq 'Fetch Authors Emails from Web of Science'
    expect(user.events.first.message).to eq 'FetchAuthorsEmailsWosJob.perform : Boom'
    expect(publication[:pub_at]).to be_falsey
  end
end
